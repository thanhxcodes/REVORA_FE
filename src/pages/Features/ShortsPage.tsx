...
import { Heart, MessageCircle, X, Send, ChevronUp, ChevronDown, Music2, ShoppingBag } from 'lucide-react';
...
function ShortVideoPlayer({ src, isActive }: { src: string; isActive: boolean }) {
  ...
  useEffect(() => {
    ...
    if (isActive) {
      ...
      video.pause();
      // Chỉ tua lại video khi video đã tải đủ dữ liệu (readyState >= 1)
...
function CommentModal({ shortId, onClose }: { shortId: number; onClose: () => void }) {
  ...
  const sendComment = async () => {
    ...
    try {
      ...
      const res = await addShortCommentAPI(shortId, inputText);
      if (res.success) {
        setComments((prev) => [...prev, res.data]); // Nối thêm bình luận vừa đăng
        setInputText('');
      }
    } catch (error: any) {
          ...
          {isLoading ? (
            ...
            comments.map((comment) => (
              <div key={comment.commentId} className="flex gap-3">
                <div className="w-9 h-9 bg-gradient-to-br from-[#2D5A3D] to-[#3D7054] rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0 overflow-hidden">
                ...
                <div className="flex-1 min-w-0 bg-white p-3 rounded-2xl shadow-sm border border-gray-100">
